...
export default function CertificadoGradeNotas({
  ...
  // render only the disciplines passed by the caller (already chunked)
  const itens = disciplinas.filter((d) => (d || '').toString().trim() !== '');
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
        <thead>
          <tr style={{ background: '#f8fafc' }}>
            <th style={{ border: '1px solid #111827', padding: '10px 8px', textAlign: 'left', width: '60%' }}>DISCIPLINA</th>
            <th style={{ border: '1px solid #111827', padding: '10px 8px', width: '13%', textAlign: 'center' }}>NOTA</th>
            <th style={{ border: '1px solid #111827', padding: '10px 8px', width: '13%', textAlign: 'center' }}>FALTA</th>
            <th style={{ border: '1px solid #111827', padding: '10px 8px', width: '14%', textAlign: 'center' }}>CH</th>
          </tr>
        </thead>
        <tbody>
          {data.map((row, i) => (
            <tr key={i} style={{ minHeight: 34 }}>
              <td style={{ border: '1px solid #111827', padding: '10px 8px', wordBreak: 'break-word' }}>
                <p style={{ margin: 0, whiteSpace: 'normal' }}>{row.disciplina}</p>
              </td>
              <td style={{ border: '1px solid #111827', padding: '10px 8px', textAlign: 'center' }}>
                <input
